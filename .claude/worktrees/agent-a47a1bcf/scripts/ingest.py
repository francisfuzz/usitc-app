...
def ingest_chapter(db, chapter_num: int, data: list) -> tuple:
...
def main():
    ...
    try:
        ...
        for ch in sorted(chapter_data.keys()):
            entries, duplicates = ingest_chapter(db, ch, chapter_data[ch])
            total_entries += entries
            total_duplicates += duplicates

        db.commit()

        print(f"Loaded {total_entries} entries across {len(chapter_data)} chapters")
